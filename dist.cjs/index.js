...
/**
   ...
   */
const generateAsync = (inputOriginal, output, options = {}) => new Promise((resolve, reject) => {
    const input = inputOriginal;
    if (!fs__namespace.existsSync(input))
        reject({ error: 'file doesnot exist please make sure you are using the right path' });
    // Check for supported output format
    const extOutput = path__namespace.extname(output).toLowerCase().replace('.', '');
    const extInput = path__namespace.extname(input).toLowerCase().replace('.', '');
    const fileNameOrignal = path__namespace.basename(input, '.' + extInput);
    if (extOutput != 'gif' && extOutput != 'jpg' && extOutput != 'png') {
        reject({ error: 'extension not supported, use png, gif, jpg' });
    }
    let fileType = 'other';
    root: for (const index in mimedb) {
    ...
    fs__namespace.lstat(input, (error, stats) => {
        if (error)
            reject(error);
        if (!stats.isFile()) {
            ...
            if (fileType == 'video') {
                ...
                node_child_process.execFile('ffmpeg', ffmpegArgs, (err) => {
                    if (err)
                        reject(err);
            ...
            if (fileType == 'image') {
                ...
                node_child_process.execFile('convert', convertArgs, (err) => {
                    if (err)
                        reject(err);
                    resolve({ thumbnail: output });
                });
            ...
            if (fileType == 'other') {
                const tempPDF = path__namespace.join(options.pdfPath || '', fileNameOrignal + '.pdf');
                node_child_process.execFile('unoconv', ['-e', 'PageRange=1', '-o', tempPDF, input], () => {
                    if (error)
                        reject(error);
                    ...
                    node_child_process.execFile('convert', convertOtherArgs, (err) => {
                        if (err)
                            reject(err);
                        if (!options.pdf || options.pdf == undefined) {
                            fs__namespace.unlink(tempPDF, (e) => {
                                if (e)
                                    reject(e);
                                resolve({ thumbnail: output });
                            });
...
/**
   ...
   */
const generateSync = (inputOriginal, output, options = {}) => new Promise((resolve, reject) => {
    const input = inputOriginal;
    if (!fs__namespace.existsSync(input))
        reject({ error: 'file doesnot exist please make sure you are using the right path' });
    // Check for supported output format
    ...
    try {
        const stats = fs__namespace.lstatSync(input);
        if (!stats.isFile()) {
            reject(false);
        }
    }
    ...
    if (fileType == 'video') {
        try {
            ...
            node_child_process.execFileSync('ffmpeg', ffmpegArgs);
            resolve({ thumbnail: output });
        }
        catch (e) {
            reject(e);
    ...
    if (fileType == 'image') {
        try {
            ...
            node_child_process.execFileSync('convert', convertArgs);
            resolve({ thumbnail: output });
        }
        catch (e) {
            reject(e);
    ...
    if (fileType == 'other') {
        try {
            const tempPDF = path__namespace.join(options.pdfPath || '', fileNameOrignal + '.pdf');
            node_child_process.execFileSync('unoconv', ['-e', 'PageRange=1', '-o', tempPDF, input]);
            const convertOtherArgs = [tempPDF + '[0]', output];
            if ((options.width || 0) > 0 && (options.height || 0) > 0) {
                if (options.keepAspect) {
                    convertOtherArgs.splice(0, 0, '-resize', options.width + 'x' + options.height);
            ...
            node_child_process.execFileSync('convert', convertOtherArgs);
            if (!options.pdf || options.pdf == undefined) {
                try {
                    fs__namespace.unlinkSync(tempPDF);
                    resolve({ thumbnail: output });
                }
                catch (e) {
                    reject(e);
